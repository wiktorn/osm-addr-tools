...
class GUGiK(AbstractImport):
    ...
    def _convert_to_address(self, soup) -> Address:
        ...
        addr_kv[str_normalize('NUMER_PORZADKOWY')] = self.__NUMER_RE.sub('\\1/\\2',
...
class GISNET(AbstractImport):
    ...
    def divide_bbox(minx, miny, maxx, maxy):
        ...
        return [
            ...
            for x in range(math.floor(minx * GISNET.__PRECISION), math.ceil(maxx * GISNET.__PRECISION),
                           GISNET.__MAX_BBOX_X * GISNET.__PRECISION)
            for y in range(math.floor(miny * GISNET.__PRECISION), math.ceil(maxy * GISNET.__PRECISION),
    ...
    def fetch_tiles(self):
        ...
        for i in self.divide_bbox(*bbox):
            ...
            if doc is not None:
                ...
                raise ValueError(
                    'No data returned from GISNET possibly to wrong scale. Check __MAX_BBOX_X, __MAX_BBOX_Y, HEIGHT and WIDTH')
        # take latest version for each point (version is last element after dot in id_)
        ret = [max(v, key=lambda z: z.id_) for v in groupby(ret, lambda z: z.id_.rsplit('.', 1)[0]).values()]
        return ret
