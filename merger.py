...
class Merger(object):
    ...
    def mark_not_existing(self):
        ...
        to_delete = (
            set(
                filter(
                    lambda x: any(
                        map(
                            lambda y: self._import_area_shape.contains(y.center),
                            self.osmdb.getbyaddress(x),
                        )
                    ),
                    tqdm.tqdm(
                        self.osmdb.getalladdress(),
                        desc="[8/14] Looking for not existing addresses",
                    ),
                )
            )
            - imp_addr
        ...
        for addr in filter(any, to_delete):  # type: OsmDbEntry
            ...
            for node in filter(
                ...
                if self._import_area_shape.contains(node.shape) and not (
                    (
                        "e-mapa.net" in self.source_addr
                        and node.source != self.source_addr
                        and "e-mapa.net" in node.source
                    )
                    or (
                        self.source_addr == "emuia.gugik.gov.pl"
                        and "e-mapa.net" in node.source
                    )
                ):
                    ...
                    self.__log.debug(
                        "Marking node to delete - address %s does not exist: %s, %s",
                        addr,
                        node.osmid,
                        str(node.entry),
                    )
    ...
    def _merge_one_address(
        self, building: typing.Dict[str, typing.Any], addr: OsmDbEntry
    ):
        ...
        if addr.get_fixme() and not addr.is_new():
            ...
            for (key, value) in addr.get_tag_soup().items():
                ...
                if oldval and oldval != value:
                    self.__log.info(
                        "Changing tag: %s from %s to %s for address: %s",
                        key,
                        oldval,
                        value,
                        addr.entry,
    ...
    def _merge_addresses_buffer(self, buf=0, message=""):
        ...
        buildings = dict(
            ((x["type"], x["id"]), x)
            for x in self.asis.elements
            if x["type"] in ("way", "relation")
        )

        self.__log.info(
            "Merging %d addresses with buildings",
    ...
    def _merge_building_with_addresses(
        self,
        _id: str,
        building: typing.Dict[str, typing.Any],
        nodes: typing.List[OsmDbEntry],
    ):
        def building_tag(tag: str) -> str:
            return building["tags"].get(tag, "")

        if len(nodes) == 1 or all(
            map(
            ...
            if building_tag("addr:housenumber") and not (
                nodes[0].similar_to(
                    self.osmdb.get_by_id(building["type"], building["id"])
                )
                and nodes[0].street
                == self.osmdb.get_by_id(building["type"], building["id"]).street
            ):
                # if building has different address, than we want to put
                self.__log.info(
                    "Skipping merging address: %s, as building already has an address: %s.",
                    str(nodes[0].entry),
                    OsmAddress.from_soup(
                        building, location=shapely.geometry.Point(0, 0)
                    ),
                )
                ...
                self.__log.debug(
                    "Merging address %s with building %s", str(nodes[0].entry), _id
                )
                for node in nodes:
    ...
    def _prepare_merge_list(
        self, buf, message=""
    ) -> typing.DefaultDict[str, typing.List[OsmAddress]]:
        """
        ...
        for addr in tqdm.tqdm(
            [
                x
                for x in (
                    self.osmdb.get_by_id(x["type"], x["id"])
                    for x in self.asis.elements
            ...
            if (
                addr.only_address_node()
                and addr.state != "delete"
                and (self._import_area_shape.contains(addr.center))
            ):
                ...
                candidates = list(
                    itertools.islice(
                        (
                            x
                            for x in self.osmdb.nearest(addr.center, num_results=1000)
                            if x.objtype in ("way", "relation")
                        ),
                        0,
                        20,
                ...
                candidate_within = next(
                    (
                        x
                        for x in candidates
                        if x.objtype == "relation"
                        and x.osmid != addr.osmid
                        and addr.center.within(
                            x.buffered_shape(buf) if buf else x.shape
                        )
                    ),
                    None,
                )
                                ...
                                itertools.islice(
                                    (
                                        ...
                                        for x in (
                                            ...
                                            for x in self.imp_index.nearest(
                                                (
                                                    candidate_within.center.y,
                                                    candidate_within.center.x,
                                                )
                                                * 2,
                                                20,
                                            )
                                        )
                                        if candidate_within.buffered_shape(
                                            buf
                                        ).contains(x.center)
                                    ),
                                    0,
    ...
    def _get_osm_xml(self, nodes, log_io=None):
        return E.osm(
            ...
            E.meta(
                osm_base=""
            ),  # self.asis.get('osm3s', {}).get('timestamp_osm_base', '')),
            *tuple(map(OsmAddress.to_osm_soup, nodes)),
            version="0.6",
            generator="import adresy merger.py"
        )
    ...
    def get_incremental_result(self, log_io=None):
        ...
        return lxml.etree.tostring(
            self._get_osm_xml(nodes, log_io),
            pretty_print=True,
    ...
    def handle_one_street_name_change(self, osm_addr, imp_addr) -> bool:
        if (
            osm_addr
            and osm_addr.housenumber.upper().replace(" ", "")
            == imp_addr.housenumber.upper().replace(" ", "")
    ...
    def handle_street_name_changes(self):
        ...
        for entry in tqdm.tqdm(
            self.impdata, desc="[6/14] Detecting street name changes"
        ):  # type: Address
                ...
                (
                    ...
                    for x in itertools.chain(
                        itertools.islice(
                            (
                                x
                                for x in self.osmdb.nearest(
                                    entry.center, num_results=100
                                )
                                if x.objtype == "relation"
                            ),
                            0,
            ...
            if not self.handle_one_street_name_change(candidate, entry):
                ...
                candidate = next(
                    itertools.takewhile(
                        ...
                        (
                            x
                            for x in self.osmdb.nearest(entry.center, num_results=1000)
    ...
    def mark_all_nodes_visible(self):
        for obj in self.osmdb.get_all_values():
            if (
                obj.housenumber
                and obj.entry["type"] == "node"
                and obj.shape.within(self._import_area_shape)
            ):
...
""" % (
    ...
    index = utils.osmshapedb.get_geometries(
        overpass.query(get_referenced_objects(query), desc="get_addresses")
    )
    return index
...
def get_addresses_terc(terc):
    ...
    """
    return utils.osmshapedb.get_geometries(
        overpass.query(get_referenced_objects(query, prefix), desc="get_addresses")
    )
...
""" % (
    ...
    boundaries = tuple(
        ...
        if x["type"] == "relation" and x["tags"].get("teryt:terc", "") == terc
    )
    if len(boundaries) > 1:
        __log.error(
            "More than one relation found with terc: %s. Names: %s. Fix before continuing",
            terc,
            ", ".join(map(lambda x: x["tags"].get("name"), boundaries)),
        )
        sys.exit(1)
    if len(boundaries) == 1:
        rel = boundaries[0]
        ...
        rel = tuple(
            ...
            for x in index.elements
            if x["type"] == "relation"
            and x["tags"].get("teryt:terc", "").startswith(terc)
        )[0]
    __log.info(
        "Loading shape of import area - relation id: %s, relation name: %s",
        rel["id"],
        rel["tags"].get("name"),
    )
...
def main():
    ...
    source_group.add_argument(
        "--impa",
        help='name of iMPA service, use "milawa" when the address is milawa.e-mapa.net',
    )
    source_group.add_argument(
        "--import-file",
    ...
    source_group.add_argument(
        "--gugik_gml",
        help="Import address data from gugik. Provide filename as argument.",
    )
    source_group.add_argument(
        "--gisnet",
    ...
    if args.import_file:
        data_func = lambda: list(
            map(lambda x: Address.from_json(x), json.load(args.import_file))
        )
    else:
