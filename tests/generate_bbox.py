    ...
    for test in sorted(directory.iterdir()):
        if test.is_dir() and (test / "osm.xml").is_file():
            with open(test / "osm.xml", "r") as f:
                ...
                if len(tree.findall("//bounds")) == 0 and len(tree.findall("//way")) > 0:
                    print()
                    print(test / "osm.xml")
                    for elem in tree.getroot().iterchildren():
                        if elem.tag == 'way':
                            positions = [
                                (node.get('lon'), node.get('lat')) for node in (
                                    tree.find('//node[@id="{}"]'.format(node.get('ref'))) for node in elem.iterchildren('nd')
                                )
                            ]
