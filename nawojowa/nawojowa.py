...
def get_number(housenumber):
    res = requests.post("http://hosting18.vpo9.iat.pl/adres1.php",
...
def get():
    res = Overpass().query("""[out:xml][timeout:300];
...
out meta;""")
    rv = dict(
        (x, get_number(x)) for x in (x.tags['addr:housenumber'] for x in itertools.chain(res.get_nodes(), res.get_ways()) if 'addr:housenumber' in x.tags)
    )
    ...
    for node in itertools.chain(et.iter('node'), et.iter('way')):
        ...
        try:
            if new and new['addr:street'] and new['addr:housenumber']:
                ...
                if node.get('lat'):
                    new_node.set('lat', str(float(node.get('lat')) + .0000075))
                    new_node.set('lon', str(float(node.get('lon')) + .000025))
                else:
                    center = node.find('center')
                    new_node.set('lat', str(float(center.get('lat'))))
                    new_node.set('lon', str(float(center.get('lon'))))

                for i in ('addr:place', 'addr:housenumber', 'source:addr', 'addr:postcode'):
                ...
                node.find("tag[@k='addr:housenumber']").set('v', new['addr:housenumber'])
