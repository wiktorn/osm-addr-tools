...
def get_dict(keyname, valuename, coexitingtags=None):
    ...
    with tempfile.NamedTemporaryFile() as temp_file:
        with TqdmUpTo(
            ...
            unit_scale=True,
            miniters=1,
            desc="Retriving {} -> {} from Overpass".format(keyname, valuename),
        ) as t:
            urllib.request.urlretrieve(
                get_nodes_ways_with_tags(tags, "xml"),
        ...
        for (_, elem) in tqdm.tqdm(
            lxml_iter_cleaner(
                lxml.etree.iterparse(
                    temp_file.name, events=("end",), tag=("node", "way")
                )
            ),
            ...
            tags = dict(
                (x.get("k"), x.get("v"))
                for x in elem.iter("tag")
                if x.get("k") in (keyname, valuename)
            )
    ...
    inconsistent = dict(
        (x[0], x[1].keys()) for x in filter(lambda x: len(x[1]) > 1, ret.items())
    )
    for (key, values) in inconsistent.items():
        __log.info(
            "Inconsitent mapping for %s = %s, values: %s",
            keyname,
            key,
            ", ".join(values),
        )
...
def stored_dict(fetcher, filename):
    try:
        ...
        __log.error(
            "Can't read a file: %s, starting with a new one", filename, exc_info=True
        )
        data = {"time": 0}
    if not DISABLE_UPDATE and data["time"] < time.time() - 21 * 24 * 60 * 60:
        try:
            ...
            __log.warning(
                "Using dictionary from: %s", time.asctime(time.localtime(data["time"]))
            )
            return data["dct"]
        data["dct"] = new
        data["time"] = time.time()
...
__DB_OSM_SIMC_POSTCODE = os.path.join(
    tempfile.gettempdir(), "osm_teryt_simc_postcode_v1.db"
)
__mapping_symul = {}
__mapping_simc = {}
__teryt_ulic = {}
...
def __init():
    ...
    if not __is_initialized:
        with __init_lock:
            if not __is_initialized:
                __mapping_symul = stored_dict(
                    lambda: get_dict("addr:street:sym_ul", "addr:street"),
                    __DB_OSM_TERYT_SYMUL,
                )
                __mapping_simc = stored_dict(
                    lambda: get_dict("teryt:simc", "name", ["place"]),
                    __DB_OSM_TERYT_SIMC,
                )  # check based on place names, not addresses
                __teryt_ulic = converters.teryt.UlicCache().get_cache()
                __mapping_simc_postcode = stored_dict(
                    lambda: get_dict("teryt:simc", "addr:postcode", ["place"]),
...
def mapstreet(strname, symul):
    ...
    def check_and_add_cecha(street):
        ...
        if teryt_entry and teryt_entry.cecha:
            teryt_nazwa = (
                teryt_entry.nazwa[5:].strip()
                if teryt_entry.nazwa.startswith("Ulica")
                and teryt_entry.cecha.upper() == "ULICA"
            ...
            if street.upper().startswith(teryt_entry.cecha_orig.upper()):
                ...
                street = street.strip()
            if (
                street.upper().startswith("UL.")
                and teryt_entry.cecha_orig.upper() == "UL."
            ):
                street = street[3:].strip()
            if (
                street.upper().startswith("ULICA")
                and teryt_entry.cecha.upper() == "ULICA"
            ):
                street = street[5:].strip()

            if not street.upper().startswith(
                teryt_entry.cecha.upper()
            ) and not street.upper().startswith(
                __CECHA_MAPPING.get(teryt_entry.cecha_orig, "").upper()
            ):
                __log.debug(
                    "Adding TERYT.CECHA=%s to street=%s (addr:street:sym_ul=%s)"
                    % (__CECHA_MAPPING.get(teryt_entry.cecha_orig, ""), street, symul)
                )
                return "%s %s" % (
                    __CECHA_MAPPING.get(teryt_entry.cecha_orig, ""),
                    street,
                )
        else:
    ...
    try:
        ...
        __log.log(
            log_level,
            "mapping street %s -> %s, TERYT: %s (addr:street:sym_ul=%s) "
            % (strname, ret, teryt_entry.nazwa if teryt_entry else "N/A", symul),
        )
        return ret
    except KeyError:
        try:
            ...
            if len(ret) > 1:
                __log.info(
                    "Inconsitent mapping for addr:street:sym_ul = %s. Original value: %s, TERYT: %s, "
                ...
                return strname
            ret = check_and_add_cecha(
                next(iter(ret.keys()))
            )  # check and add for first and only key
            if ret != strname:
                log_level = logging.INFO
                if max(strname.split(" "), key=len) in ret:
                    log_level = logging.DEBUG
...
def mapcity(cityname, simc):
    ...
    try:
        ...
        if len(ret) > 1:
            __log.info(
                "Inconsitent mapping for addr:city:simc = %s. Original value: %s, OSM values: %s. "
        ...
        if ret != cityname:
            __log.info(
                "mapping city %s -> %s (addr:city:simc=%s)" % (cityname, ret, simc)
            )
        return ret
    except KeyError:
...
def mappostcode(postcode, simc):
    ...
    try:
        ...
        if len(ret) > 1:
            __log.info(
                "Inconsistent mapping for teryt:simc = %s to postcode. OSM values: %s",
                simc,
                ", ".join(ret),
            )
            return postcode
