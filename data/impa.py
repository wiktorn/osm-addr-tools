...
class iMPA(AbstractImport):
    ...
    def fetch_point(self, wms_addr, w, s, e, n, pointx, pointy, layer="punkty"):
        ...
        if "?" in wms_addr:
            ...
            self.__log.warning(
                "JOSM layer: %s&%s&SRS={proj}&WIDTH={width}&HEIGHT={height}&BBOX={bbox}"
        ...
        with tempfile.NamedTemporaryFile() as temp:
            with TqdmUpTo(
                unit="B", unit_scale=True, miniters=1, desc="Downloading from iMPA"
            ) as t:
                urllib.request.urlretrieve(
                    url, filename=temp.name, reporthook=t.update_to
                )
            data = temp.read()
        return data
    ...
    def _convert_to_address_html(self, soup):
        ...
        try:
            ...
            if float(lon) < 14 or float(lon) > 25 or float(lat) < 49 or float(lat) > 56:
                self.__log.warning(
                    "Point out of Polish borders: (%s, %s), %s, %s, %s",
                    lat,
                    lon,
                    kv.get("Miejscowosc", ""),
                    kv.get("Ulica", ""),
    ...
    def fetch_tiles(self):
        html = self.fetch_point(
            ...
            *self.get_bbox_2180(),
            pointx=0,
            pointy=0  # sprawdź punkt (0,0) i tak powinno zostać zwrócone wszystko
        )
        if self.__USE_GML:
            ret = [
                self._convert_to_address_gml(x)
                for x in tqdm.tqdm(
                    BeautifulSoup(html, "xml").find_all("punkty_feature"),
                    desc="Conversion",
                )
            ]
        else:
            ret = [
                self._convert_to_address_html(x)
                for x in tqdm.tqdm(
                    BeautifulSoup(html, "lxml-xml").find_all("Adres"), desc="Conversion"
                )
            ]
        return ret
