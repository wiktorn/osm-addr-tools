...
if hasattr(pyproj, "Transformer"):
    __WGS84_EPSG2180_TRANSFORMER = pyproj.Transformer.from_proj(
        __WGS84, __EPSG2180
    )  # type: pyproj.Transformer
    __EPSG2180_WGS84_TRANSFORMER = pyproj.Transformer.from_proj(
        __EPSG2180, __WGS84
    )  # type: pyproj.Transformer
    ...
    def get_transformer_to_wgs(srs) -> pyproj.Transformer:
        return pyproj.Transformer.from_proj(get_proj(srs), __WGS84).transform
    ...
    def get_transformer_to_wgs(srs):
        return functools.partial(pyproj.transform, get_proj(srs), __WGS84)
...
def srs_to_wgs(srs, lon, lat):
    if srs.startswith("urn:ogc:def:crs:"):
        srs = srs[16:].replace("::", ":")
    return get_transformer_to_wgs(srs)(lon, lat)
...
def convert_to_osm(lst):
    ...
    osm = ret.new_tag(
        "osm", version="0.6", upload="false", generator="punktyadresowe_import.py"
    )
    ret.append(osm)
...
class Address(object):
    ...
    def __init__(
        ...
        assert all(
            map(
                lambda x: isinstance(getattr(self, x, ""), str),
                (
                    "housenumber",
                    "postcode",
                    "street",
    ...
    def as_osm_soup(self, node_id):
        ...
        node = ret.new_tag(
            ...
            action="modify",
            visible="true",
            lat="{:07f}".format(self.location["lat"]),
            lon="{:07f}".format(self.location["lon"]),
        )
    ...
    def similar_to(self, other):
        ...
        if self.id_ and other.id_ and self.id_ == other.id_:
            return True
        ret &= other.housenumber.upper().replace(
            " ", ""
        ) == self.housenumber.upper().replace(" ", "")
    ...
    def from_osm_xml(elem):
        tags = dict((x.get("k"), x.get("v")) for x in elem if x.tag == "tag")
        if "addr:place" in tags and "addr:city" not in tags:
            tags["addr:city"] = tags["addr:place"]
        tags["location"] = {"lon": elem.get("lon"), "lat": elem.get("lat")}
        tags["id"] = elem.get("id")
...
out;
            ...
            data = osmshapedb.get_geometries(
                overpass.query(query, desc="base:getboundary")
            )
            try:
    ...
    def _check_duplicates_in_import(self, data):
        ...
        for (addr, occurances) in sorted(
            ...
            for (a, b) in filter(
                lambda x: distance(x[0].center, x[1].center) < 10,
        ...
        for (addr, occurances) in sorted(
            ...
            if any(
                ...
                self.__log.warning(
                    "Address points doesn't fit into 100m circle. Points count: %d",
                    len(occurances),
                )
                for i in occurances:
    ...
    def _check_mixed_scheme(self, data):
        dups = groupby(
            (x for x in data if x.simc), lambda x: x.simc, lambda x: bool(x.street)
        )

        dups_count = dict((k, len(_filter_ones(v))) for k, v in dups.items())
        dups = dict((k, len(_filter_ones(v)) / len(v)) for k, v in dups.items())
        dups = dict(
            (k, v) for k, v in filter(lambda x: 0 < x[1] and x[1] < 1, dups.items())
        )

        for i in filter(lambda x: not bool(x.street) and x.simc in dups.keys(), data):
            i.add_fixme(
